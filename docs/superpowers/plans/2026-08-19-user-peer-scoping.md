...
- [ ] **Step 1: Add `listPeerIds` to `WorkspaceMembersRepository`**
...
- [ ] **Step 2: Add the `ids` filter to `UsersRepository.list` and `countFiltered`**
...
- [ ] **Step 4: Ask the user to commit** (do not commit automatically).
...
- [ ] **Step 1: Write the failing unit tests** — create `apps/api/src/modules/users/services/users.service.spec.ts` (mirror the fixture style of `apps/api/src/modules/workspaces/services/workspaces.service.spec.ts`):
...
- [ ] **Step 3: Implement the service gates** — in `apps/api/src/modules/users/services/users.service.ts`, add a private gate helper and change the four public entry points:
...
- [ ] **Step 4: Wire the controller** — in `apps/api/src/modules/users/controllers/users.controller.ts`:
...
- [ ] **Step 1: Fix the borderGlow keyframe**
...
- [ ] **Step 3: Live smoke over the running stack** — start the API against the local Postgres (postgres/redis are still running; ports 3000/4000 are free):
...
- [ ] **Step 4: Stop the API dev server** (kill the `:4000` listener; use `ss -tlnp` to find the PID — never `pkill -f "next-server"`).

- [ ] **Step 5: Ask the user to commit** (globals.css) — then the final commit batch is complete and the work can be pushed (Vercel/Render deploy automatically).