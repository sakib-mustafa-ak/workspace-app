...
description: Dark-first collaborative workspace. Deep ink surfaces, calm instrument-blue accent, flat at rest with quiet glow on interaction.
colors:
  primary: "#2563eb"
...
The canvas is the product. Workspace OS is a real-time collaboration surface, and the UI behaves like a well-kept desk in a dark studio: everything needed is within reach, the lighting is dim so the work stands out, and nothing on the desk competes with the paper you're drawing on. The interface is a quiet instrument tuned for focus — dark ink surfaces recede, the instrument-blue accent marks only the active moment, and depth appears when you reach for something, not at rest.

The aesthetic is calm and precise: deep slate-ink panels layered tonally, hairline borders at the seams, and a single cool blue that reads as signal rather than personality. Density is compact but airy — 12px body text for meta, 14px for work, generous 8px radii on controls, 12–16px on containers. Motion is quick and utilitarian (150–350ms ease-out); nothing bounces, everything settles. This is an instrument, not a toy.

Confirmed visual rejections: no candy/neon dark mode, no purple or multi-hue gradients, no glassmorphism on every surface (frosted blur is reserved for hero/auth panels), no playful animation.

**Key Characteristics:**
- Dark-first ink surfaces (surface-950 page, surface-900 panels, surface-800 raised) with hairline slate borders
- Single instrument-blue accent (primary-600/500/400) for actions, focus, active state, and the logo mark
...
A single cool accent ("Instrument Blue") over a deep slate-ink neutral ramp. Blue is used sparingly — it is signal, not fill.

### Primary
- **Instrument Blue** (#2563eb, primary-600): The action color. Primary buttons, active pill backgrounds, focus accents, logo gradient end.
- **Bright Signal** (#3b82f6, primary-500): Hover state for primary actions, gradient start of the logo mark, glow shadows.
- **Soft Signal** (#60a5fa, primary-400): Active nav icons, secondary text accents, avatar gradients.
- **Deep Signal** (#1d4ed8, primary-700): Gradient end of the logo mark, deep hover tints.

### Neutral
- **Deep Ink** (#020617, surface-950): Page background, input fill. The desk itself.
- **Ink Panel** (#0f172a, surface-900): Cards, sidebar gradient base, dropdown surfaces.
- **Raised Slate** (#1e293b, surface-800): Active nav items, hover fills, input borders, panel seams.
- **Edge Slate** (#334155, surface-700): Stronger borders, ghost button strokes, workspace avatar chips.
- **Snow** (#f1f5f9, surface-100): Primary text on dark.
- **Mist** (#94a3b8, surface-400): Muted text — meta, inactive nav, placeholders.
- **Steel** (#64748b, surface-500): Faint text, section labels, icon strokes at rest.
...
### Named Rules
**The Signal Rule.** Blue is reserved for the active moment: actions, focus, selection, and the logo. Resting surfaces are ink and slate only — a screen with blue everywhere has lost its signal.
...
Hybrid, tonal-first: depth at rest comes from stacking ink tones (page 950 → panel 900 → raised 800), not shadows. Shadows appear only as response — hover glow, float, or focus. Frosted blur (backdrop-blur-xl) appears only on hero/auth glass panels, never on working cards.
...
- **Hover glow** (`0 10px 15px -3px rgb(37 99 235 / 0.25)`, shadow-lg + shadow-primary-600/25): Primary buttons and interactive accents on hover — blue-tinted glow marks reachability.
...
- **State:** unread counters are solid signal-red; workspace initials are 6px-corner squares with slate-700 fill and mist text; status chips use tinted backgrounds (red-500/10, primary-600/10) with matching text (red-400, primary-300).

### Cards / Containers
- **Corner Style:** 12px (rounded-xl) standard; 16px (rounded-2xl) for hero/auth glass.
- **Background:** ink-panel (surface-900) solid or gradient (from-surface-900 to-surface-900/60).
...
### Inputs / Fields
- **Style:** 1px surface-800 stroke, deep-ink fill (surface-950), 8px corners, 12px text, steel placeholder.
- **Focus:** the stroke shifts to primary-500/50 — a calm blue seam, no ring glow, no background change.
...
- **Active:** raised-slate fill (surface-800), white text, shadow-sm, icon shifts to soft-signal blue. The active row is the only blue-accented nav element.
...
### Do:
- **Do** start every page from the deep-ink backdrop (surface-950) and build hierarchy with tonal layering (900 → 800).
- **Do** reserve blue for actions, focus strokes, active rows, and the logo mark — and use primary-600 as the default action fill.
...
- **Do** keep resting surfaces flat — reach for the hover glow (blue-tinted) only on interactive elements.
...
- **Don't** use blue as a text color for body copy; text accents stay at primary-300/400 and only for icons or emphasis.
- **Don't** introduce a second accent hue, purple gradients, or neon — the system is instrument-blue over ink.
